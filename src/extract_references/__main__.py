...
Examples:
  ...
  python3 -m src.extract_references paper.pdf --no_claims   # references only, skip claim extraction
...
""",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Max citations parsed concurrently, and max claim-extraction paragraphs parsed concurrently.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--no_claims",
        action="store_true",
        help="Skip Step 5 (LLM-based claim extraction from the body text) — references only.",
    )
    parser.add_argument(
        "--mineru_cmd",
        type=str,
    ...
    try:
        result = asyncio.run(pipeline.run(str(pdf_path)))
        ...
        print(
            f"\n[Success] Processed {len(result.references)} citation(s), "
